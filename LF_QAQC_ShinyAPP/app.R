...
server <- function(input, output, session) {
  ...
  # Reactive expression to read Excel file
  dataInfo <- reactive({
    req(input$file)
    
    tryCatch({
      print("Reading Excel file...")
      excel_file <- read_excel(input$file$datapath)
  ...
  # Update block dropdown choices based on the selected species and seed lot
  observe({
    tryCatch({
      ...
      if ("block_num" %in% colnames(speciesSeedlotData)) {
        ...
        updateSelectInput(session, "block_num", choices = block_choices, selected = block_choices)
      } else {
  ...
  output$measurementPlot <- renderPlot({
    ...
    tryCatch({
      ...
      ggplot(plot_data, aes(x = date, y = get(input$measurement), color = interaction(seedlot, block_num), linetype = factor(block_num))) +
